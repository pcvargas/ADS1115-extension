# modules/ADS1115.py — Driver MicroPython para KidBright32
...
def _wr(r,v): _i2c.writeto(_addr,bytes([r,(v>>8)&0xFF,v&0xFF]))
def _rd(r):
    _i2c.writeto(_addr,bytes([r])); b=_i2c.readfrom(_addr,2)
    return (b[0]<<8)|b[1]
...
def _raw(ch):
    ...
    _wr(_REG_CFG,cfg)
    from time import sleep_ms; sleep_ms(10)
    while not (_rd(_REG_CFG)&0x8000): sleep_ms(1)
    v=_rd(_REG_CONV); return v-65536 if v>32767 else v

def begin(addr=0x48):
    global _i2c,_addr; _addr=addr; _i2c=_bus()
